...
export class DB {
    ...
    async put(r2: R2Bucket, {
        ...
        for (let i = 0; i < 2; i++) {
            try {
                ...
                await db.prepare(SQL.fixed.put_img_data).bind(hash, path, size, type).run()
    ...
    async get_hash(album: string | null, scale: string | null, rid: number | null): Promise<string | null> {
        const db = this.db
        if (rid) {
    ...
    async get_path(hash: string, size: string, type: string): Promise<string | null> {
        return await this.db.prepare(SQL.fixed.get_img_path).bind(hash, size, type).first('Path')
    }
    ...
    async in_album(hash: string): Promise<boolean> {
...
const getByRid = async (db: D1Database, rid: number, album: string | null, scale: string | null) => {
...
const getHash = async (db: D1Database, query: string, ...bindings: unknown[]): Promise<string | null> => {
    ...
    console.log(await db.prepare(query).bind(...args).all())
